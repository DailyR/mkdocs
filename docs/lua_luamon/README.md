...
- 继续调试
	...
	* 由于任务多，零碎，时间会被碎片化，有时候第二天继续的时候不知道从何着手，这里记录下步骤
	* 第二天来的时候照例先 更新 服务端，客户端（包括exe和Unity），如果要删档的删档
	* 模拟好环境后，开始跑两个号（用现有的自动化测试跑到对应的等级）
	* 一个号用来在更新之后的unity工程里跑号，另外一个用于exe的调试（结合两者的优缺点使得调试环境和步骤较为简单）
	* 获取到这两个账号和对应的登录环境后（exe和unity），打开sublime和web端的luamon调试器
	* 一般这个时候luamon只保留两个调试的script，一个是trunk test,就是主干逻辑脚本，类似于Guide_chapter_regress.lua这种，
	另一个target test，是用来临时调试的小脚本代码，这里就是调试的主要执行场所。
	* 有了上述这些之后，在内网条件下面就可以开始用Print（此处主要是ferPrint）来进行调试了，开始梳理逻辑和思路，就很多时候很多事情，不是害怕它有多复杂，而是害怕它没有规律，一旦有了规律，抓住规律就能理解它的本质核心问题。
	* target test开始调试
	* 如果是进入到对应的全屏View，既可以用这个UiMgr来判断了，我们的游戏，大部分是通过UI，来控制各种各样的Object，部队，城池对象，点击之后出现操作UI，主界面UI这些来做常见的操作，抽象与具体，可以通过这些来转换，完成很多操作。

- 内网来不及同步的就直接放在内网就可以了
	- CampView_test.lua 募兵营的测试用例，基本用例可以在内网里面找到对应的testcase
	- 思考：对应的View，比如特定的view是在传入一些固定参数才能生效的，那么这个view的源码要找到获取对象和适用实例
	- 方法的调用需要注意:和.的区别
	- 前期不要要求所有的文件都装进一个lua文件里面
	- 分批次，比如1级主堡->2级主堡，2级主堡->3级主堡作为一个lua文件
		- 其中，2到3级的这个lua文件可以继承之前的1-2级文件的使用结果。就是账号和状态的公用化。
	- 等成熟后，就是上面3个类型的测试用例都完成后，再重新分拆和组装

- 阶段性完成内网同步外网
	- 主要是CampView募兵营相关，插入了正确的位置和时机
	- 
