...
- 思考过程
	...
	- 这个时候还需要一个漂亮的跳转如何汇总页
		...
		- 引入排好版之后，用files_parse函数把约定好的客户端性能名字给按照规律进行解析，这一段是字符串处理，这里就用之前的list.append， 利用条件语句，灵活使用各种字符串处理语句，.split， [:]字符串本身的截取，装配了完对应的字符串后，用一个大的list将这些都包裹打包起来，作为返回值返回，这样后续就可以用到这个重要的函数和变量了。
